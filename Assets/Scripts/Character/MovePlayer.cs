...
public class MovePlayer : MonoBehaviour
{
    ...
    [SerializeField] List<GameObject> missingItems;
    ...
    private void UpdateMovement() {
        ...
        if (Input.GetKeyDown(KeyCode.Space)) {
            currentDir.y = jumpSpeed * Time.deltaTime;
        }
    }
    ...
    private void OnGUI() {
        if (numberOfItems > 0) { 
            GUI.Box(new Rect((Screen.width/2)-100, 10, 200, 35), "" + numberOfItems + " items missing"); 
        } 
        else { 
            GUI.Box(new Rect((Screen.width/2)-100, 10, 250, 35), "" + " Congratulation all items collected!"); 
        }
    }
